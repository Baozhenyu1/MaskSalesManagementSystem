    ...
    <a-form ref="formRegister" :form="form" id="formRegister">
      ...
      <a-form-item>
        <a-input
          ...
          placeholder="用户名"
          type="text"
          v-decorator="['username', {rules: [{ required: true, message: '请输入用户名'}, { validator: this.handleUserNameCheck } ], validateTrigger: ['change', 'blur'] }]">
        </a-input>
      </a-form-item>
      ...
      <a-row :gutter="16">
        <a-col class="gutter-row" :span="16">
          <a-form-item>
            <a-input size="large" type="text" placeholder="验证码" v-decorator="['captcha', {rules: [{ required: true, message: '请输入验证码' }, { validator: this.handleCaptchaCheck }], validateTrigger: ['blur','change']}]">
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      validateFields({ force: true }, (err, values) => {
        if (!err) {
          const postPara = { email: values['email'], username: values['username'], password: values['password'] }
          register(postPara)
